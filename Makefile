...
upx.tar.xz:
	wget -q https://github.com/upx/upx/releases/download/v3.96/upx-3.96-amd64_linux.tar.xz -O $(BUILD_DIR)/upx.tar.xz
...
install: $(BUILD_DIR)/$(NAME) geoip.dat geosite.dat $(BUILD_DIR)/upx.tar.xz
	mkdir -p /etc/$(NAME)
	mkdir -p /usr/share/$(NAME)
	rm -rf $(BUILD_DIR)/upx 
	xz -d -c $(BUILD_DIR)/upx.tar.xz | tar -x -C $(BUILD_DIR)/upx
